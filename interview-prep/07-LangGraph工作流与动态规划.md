...
评估节点（evaluate_results）通过 LLM 判断：

**评估 prompt 的核心要素**：
1. 对比计划 vs 完成：检查 `task_plan` 中的每个子任务是否都有对应的完成结果
2. 质量评估：LLM 评估每个子任务结果是否满足原始要求
3. 完整性检查：如果有依赖子任务 A 的结果，检查结果中是否包含了必要信息

**通过标准**：
- 所有计划中的子任务都已完成
- 每个子任务的结果质量可接受（不要求完美，只要合理）
- 子任务之间的衔接正确（前一步的输出被后一步正确使用了）

**不通过的场景**：
- 某个子任务 Agent 返回了不相关的内容
- 子任务输出质量太差（如代码有语法错误）
- 缺少关键信息（如规划了 5 步，只完成了 3 步）

评估节点返回不通过时 → 生成补充计划 → 重新执行。
...
**2. 评估阶段纠正**：
- evaluate_results 节点会检查子任务结果
- 如果发现某个子任务的输出覆盖了另一个子任务的内容（划分过粗），会触发重规划
...
**4. 降级兜底**：
- 如果多次重规划仍然失败，降级为直接 LLM 调用

**核心设计理念**：不追求一次完美，而是通过"规划 → 执行 → 评估 → 重规划"的循环逐步逼近。
...
1. **最大迭代次数**：`DEFAULT_MAX_ITERATIONS = 10`，超过后强制结束
2. **错误计数**：GraphState 中的 `error_count` 字段累计错误次数，达到上限后强制跳转到 generate_summary
3. **评估节点的通过条件**：设计得相对宽松——"结果合理即可"，不是完美主义
4. **降级逻辑**：在 `error_count` 超过阈值时，降级为用当前已有结果生成摘要（而非要求重做）
5. **单轮退避**：如果某一轮的所有子任务都没通过评估，不追加新任务，直接用现有结果生成摘要
